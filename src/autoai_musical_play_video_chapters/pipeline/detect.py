...
def refine_song_in_blocks(
    ...
    task_deadline: float,
    *,
    estimate_prompt_tokens: Callable[[str], int],
    system_prompt: str,
    llm_chat_json: Callable[..., dict],
    validation_error_cls: type[Exception],
    normalize_space: Callable[[str], str],
    confidence_score_to_label: Callable[[float], str],
    downgrade_confidence: Callable[[str], str],
    boundary_refine_prompt: Callable[..., str] | None = None,
) -> dict | None:
    """Refine selected block context into exact song cue boundaries."""
    block_idx = selected_block_id - 1
    refine_prompt_factory = boundary_refine_prompt or globals()["boundary_refine_prompt"]
        ...
        if ENABLE_VERIFIER:
            ...
            if edge_choice or confidence_score < 0.7:
                ...
                if verify_candidates:
                    ...
                    verify_prompt = refine_prompt_factory(
                        song,
                        prev_song,
                        next_song,
                        search_cue_id,
                    ...
                    def verify_prompt_builder(state):
                        current_candidates = state.get("candidate_pool", verify_candidates)
                        current_allowed = [c["candidate_index"] for c in current_candidates]
                        prompt_text = refine_prompt_factory(
                            song,
                            prev_song,
                            next_song,
                            search_cue_id,
