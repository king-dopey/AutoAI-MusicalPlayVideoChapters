...
If a verification step requires runtime execution, external network access, or container builds, record that requirement as an **Operator Verification** task in the relevant `REFACTOR_PLAN.md` commit and stop; the operator will run those steps on a separate host.
...
2. `PYTHONPATH=src python3 -m autoai_musical_play_video_chapters --help` (or equivalent
...
- Verification command output (stdout/stderr) MUST be appended to the relevant commit's `Verification Output` block in `REFACTOR_PLAN.md` and must never be pasted into chat.
